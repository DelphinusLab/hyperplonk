...
impl<F: Field> MultilinearPolynomial<F> {
    ...
    pub fn evaluate_for_rotation(&self, x: &[F], rotation: Rotation) -> Vec<F> {
        ...
        // 2. construct poly after rotate
        let poly_rotated = MultilinearPolynomial::new(rotated_evals);

        // 3. eval rotated poly at oritinal point
        vec![poly_rotated.evaluate(x)]
    }

    pub fn evaluate_for_rotation_binary_field(&self, x: &[F], rotation: Rotation) -> Vec<F> {
        assert_eq!(x.len(), self.num_vars);
        if rotation == Rotation::cur() {
            return vec![self.evaluate(x)];
        }
...
pub fn rotation_eval<F: Field>(_x: &[F], _rotation: Rotation, evals_for_rotation: &[F]) -> F {
    // if rotation == Rotation::cur() {
