...
use crate::util::{
    arithmetic::{powers, squares, BatchInvert, FieldExt},
...
pub fn s_polys<F: FieldExt>(num_vars: usize) -> Vec<Vec<F>> {
    ...
    let vanishing = {
        ...
        powers(domain.extended_omega().pow(&[domain.n() as u64, 0, 0, 0]))
...
fn s_evals<F: FieldExt>(domain: &Radix2Domain<F>, poly: usize, x: F) -> Vec<(Query, F)> {
...
fn vanishing_eval<F: FieldExt>(domain: &Radix2Domain<F>, x: F) -> F {
    x.pow(&[domain.n() as u64, 0, 0, 0]) - F::one()
}
