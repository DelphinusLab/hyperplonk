use crate::{
    pcs::{
        ...
        Evaluation, EvaluationForShift, Point, PolynomialCommitmentScheme,
    },
    poly::{multilinear::MultilinearPolynomial, univariate::UnivariatePolynomial},
    util::{
        arithmetic::{powers, squares, variable_base_msm, BatchInvert, Field, MultiMillerLoop},
        chain, izip,
    ...
    M::G2Affine: Serialize + DeserializeOwned,
{
    ...
    fn batch_open_for_shift<'a>(
        ...
        comms: impl IntoIterator<Item = &'a Self::Commitment>,
        points: &[Point<M::Scalar, Self::Polynomial>],
        evals: &[EvaluationForShift<M::Scalar>],
        transcript: &mut impl TranscriptWrite<Self::CommitmentChunk, M::Scalar>,
    ) -> Result<(), Error>
    where
        Self::Commitment: 'a,
    ...
    fn verify(
        ...
        M::pairings_product_is_identity(&[(&pi, &(vp.s_g2()).into()), (&c, &(-vp.g2()).into())])
            .then_some(())
